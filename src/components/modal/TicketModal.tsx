...
const TicketModal: React.FC<TicketModal> = ({ onBackdropClick, isTicketModalVisible, children }: Props) => {
  ...
  return ReactDom.createPortal(
    <div>
      ...
      <div className={`fixed inset-1/4 h-2/3 bg-white text-black shadow-lg w-full max-w-screen-sm p-4 ${isTicketModalVisible ? 'opacity-100' : 'pointer-events-none opacity-0'}`}>
        <div className="modalHeader relative h-5">
          <button className="text-white absolute right-0 top:0" onClick={onBackdropClick}>
            Close
          </button>
        </div>
